...
def test_fit_when_k_greater_than_num_points_throws_error():
    ...
    with pytest.raises(
        ValueError,
        match="Number of clusters cannot be greater than number of data points.",
    ):
        kmeans.fit(X)
...
def test_assign_clusters():
    ...
    X = np.array(
        [[1, 1], [2, 1], [1, 2], [10, 10], [11, 10], [10, 11]]  # Cluster 1
    )  # Cluster 2

    # Create KMeans instance with fixed centroids
    kmeans = KMeans(n_clusters=2)
    kmeans.centroids_ = np.array(
        [[1, 1], [10, 10]]
    )  # Set centroids manually to cluster centers
...
def test_update_centroids():
    ...
    kmeans.labels_ = np.array(
        [0, 0, 0, 1, 1, 1]
    )  # First 3 points to cluster 0, last 3 to cluster 1
